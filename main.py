...
def parser(url):
    ...
    a = 1                                                           ### тут задал счетчик количества шагов в списке, грубо говоря (там, насколько помню, +-215 элементов)
    ...
    for i in range(len(clear_currencies) // 5):                     ### тут погнали бегать соответственно по элементам и их объединять
        clear_currencies_str = ' '.join(clear_currencies[a:4 + a])  ### тут и счетчик пригодился бегаем от нулевого и через каждые пять
...
def hello(message):
    bot.send_message(message.chat.id, 'Здравствуйте! Чтобы узнать курс интересующей Вас валюты, введите его название, например "usd" или "доллар":')
